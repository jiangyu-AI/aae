...
from model import GumbelVAE, PixelCNN
from model import _gumbel_mnist_arch as _mnist_arch
...
def main(config,
    ...
    mnist = input_data.read_data_sets("/scratch/jy1367/datasets/mnist", one_hot=False)
    # <<<<<<<
    ...
    with tf.variable_scope('train'):
        ...
        with tf.variable_scope('params') as params:
            pass
        net = GumbelVAE(tau, learning_rate,global_step,BETA,resized,K,D,_mnist_arch,params,'train')

    with tf.variable_scope('valid'):
        params.reuse_variables()
        valid_net = GumbelVAE(tau, None,None,BETA,resized,K,D,_mnist_arch,params,'decode')
    ...
    for step in tqdm(xrange(TRAIN_NUM),dynamic_ncols=True):
        tau_step = np.max([tau_step * np.exp(- anneal_rate * step/500), min_temperature])
        #tau_step = np.max([tau_step * np.exp(- anneal_rate * epoch), min_temperature])
        
        batch_xs, _= mnist.train.next_batch(BATCH_SIZE)
        it,loss,_ = sess.run([global_step,net.loss,net.train_op],feed_dict={x:batch_xs, tau:tau_step})
        ...
        if( it % SUMMARY_PERIOD == 0 ):
            ...
            summary = sess.run(summary_op,feed_dict={x:batch_xs, tau:tau_step})
            summary_writer.add_summary(summary,it)

        if( it % (SUMMARY_PERIOD*2) == 0 ): #Extended Summary
            batch_xs, _= mnist.test.next_batch(BATCH_SIZE)
            summary = sess.run(extended_summary_op,feed_dict={x:batch_xs, tau:tau_step})
            summary_writer.add_summary(summary,it)
...
def extract_z(MODEL,
    ...
    with tf.variable_scope('net'):
        with tf.variable_scope('params') as params:
            pass
        tau_notused = 0.5
        net = GumbelVAE(tau_notused, None,None,BETA,resized,K,D,_mnist_arch,params,'transform')
...
def get_default_param():
    ...
    return {
        ...
        'MODEL' : '/scratch/jy1367/workspace/tf-vqvae/test/log/model.ckpt-0',

        'TRAIN_NUM' : 10000, #Size corresponds to one epoch
        ...
        'DECAY_VAL' : 1.0,
        'DECAY_STEPS' : 3000, # Half of the training procedure.
